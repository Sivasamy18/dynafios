@extends('layouts/_physician_hospital', [ 'tab' => 1 ])
@section('actions')
    <a class="btn btn-default btn-generate" href="#"><i class="fa fa-refresh fa-fw"></i> Generate</a>
@endsection
@section('content')
	<div class="filters clearfix">
        <a class="active" href="{{ URL::current() }}?p_id={{$physician->practice_id}}">Hospital Report</a>
        <a class="" href="{{ URL::route('physician.paymentStatusReport', $physician->id) }}?p_id={{$physician->practice_id}}">Payment Status Report</a>
    </div>
...
<script type="text/javascript">
    ...
    $(function () {
			...
			$(document).on("change", "[name=hospital]", function(event) {
                ...
                $.ajax({
                    url:'/getContractTypesByPhysician/'+ physician_id + '/' + hospital_id + '/' + 0,
                    dataType: 'json',
