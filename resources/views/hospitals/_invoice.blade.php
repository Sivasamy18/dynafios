...
@if (count($contracts) > 0)
    <ul style="list-style:none;padding-top:15px;padding-left: 0px">
            <?php $i = 0; ?>
        @foreach ($contracts as $contract)
            <li>
                ...
                <input type="hidden" class="contract_name pull-right" id="contract_name" name="contract_name"
                       value="{{ $contract->name }}">
                <input type="hidden" class="contractId pull-right" name="contractId"
                       value="{{ $contract->id }}">
                <input type="hidden" class="is_shared_contract_{{$contract->id}} pull-right" name="is_shared_contract"
                ...
                <ul style="list-style:none">
                    ...
                    @foreach ($contract->practices as $practice)
                        <input type="hidden" class="contract_id pull-right" id="contract_id" name="contract_id"
                               value="{{ $contract->id }}">
                        <input type="hidden" class="practice_id_{{$contract->id}} pull-right"
                               id="practice_id_{{$contract->id}}" name="practice_id"
                               value="{{ $practice->id }}">
                        <input type="hidden" class="practice_name_{{$contract->id}} pull-right"
                               id="practice_name_{{$contract->id}}" name="practice_name_{{$contract->id}}"
                               value="{{ $practice->name }}">
                        ...
                        <li>
                            <div class="col-md-12" style="margin-top:15px;"><strong>Expected Practice Total
                                    : </strong><span
                                        class="dynaminc_expected_total">${{number_format($practice->expected_practice_total,2,'.','')}}</span>
                            </div>
                            <div class="col-md-12" style="margin-top:15px;"><strong>Practice Total : </strong><span
                                        class="dynaminc_paid_total">${{number_format($practice->practice_total,2,'.','')}}</span>
                            </div>
                            <ul style="list-style:none">
                                <li style="clear: both;">
                                        <?php
                                    ...
                                    <div style="width: {{$practice_width}}; float: left;">
                                        ...
                                        @endif
                                        <div class="alert alert-danger" id="invoiceDismissPractice_{{$practice->id}}"
                                        ...
                                        <ul style="list-style:none">
                                            ...
                                            @foreach ($practice->physicians as $index => $physician)
                                                ...
                                                <input type="hidden" class="physician_id_{{ $contract->id }} pull-right"
                                                       id="physician_id_{{ $contract->id }}"
                                                       name="physician_id_{{ $contract->id }}"
                                                       value="{{ $physician->id }}">
                                                <input type="hidden" class="contract_id_{{ $physician->id }} pull-right"
                                                       id="contract_id_{{ $physician->id }}"
                                                       name="contract_id_{{ $physician->id }}"
                                                       value="{{ $physician->contract_id }}">
                                                <input type="hidden"
                                                       class="physician_name_{{$physician->id}} pull-right"
                                                       id="physician_name_{{$physician->id}}"
                                                       name="physician_name_{{$physician->id}}"
                                                       value="{{ $physician->name }}">
                                                {{--                                                    <input type="hidden" class="practiceId pull-right" name="practiceId"--}}
                                                {{--                                                           value="{{ $practice->id }}">--}}
                                                <input type="hidden" class="physician_contract_id pull-right"
                                                       name="contractId"
                                                       value="{{ $physician->contract_id }}">
                                                <li style="clear: both;">
                                                    ...
                                                    <div style="width: 50%; float: left;">
                                                        @if($contract->is_shared_contract == 0)
                                                            ...
                                                            @foreach ($physician->amountPaid as $amount_paid)
                                                                ...
                                                                <input type="hidden" name="amountPaidID"
                                                                       value="{{$contract->id}}">
                                                                <input type="text"
                                                                       id="amountPaidId_{{$amount_paid->id}}"
                                                                       class="prevAmount prevAmount_{{ $physician->contract_id }}"
                                                                       name="amountPaidt"
                                                                       value="${{number_format($amount_paid->amountPaid, 2,'.','')}}"
                                                                       style="margin-top:5px;margin-bottom:5px; width: 90%;"
                                                                       placeholder="$"
                                                                       title="Amount Paid" {{$disabled}}>
                                                                    <?php /* @if($physician->remaining_amount > 0) */ ?>
                                                                <input type="checkbox"
                                                                       name="invoice_print_checkbox_paid[]"
                                                                       id="printPaidAmount_{{$amount_paid->id}}"
                                                                       value="{{$amount_paid->id}}" {{$disabled}}>
                                                                    <?php /*@else
                                                    ...
                                                    */ ?>
                                                                <input type="hidden"
                                                                       class="amtPaidId_{{$physician->contract_id}}_{{$practice->id}}"
                                                                       id="amtPaidId_{{$physician->contract_id}}_{{$practice->id}}"
                                                                       value="{{$amount_paid->id}}">
                                                                <input type="hidden" id="invoiceNo_{{$amount_paid->id}}"
                                                                       value="{{$amount_paid->invoice_no}}">
                                                                <input type="hidden"
                                                                       id="amountPaidIdOld_{{$amount_paid->id}}"
                                                                       class="prevAmountOld prevAmountOld_{{ $physician->contract_id }}"
                                                                       name="amountPaidOld"
                                                                       value="${{number_format($amount_paid->amountPaid, 2,'.','')}}"
                                                                       style="margin-top:5px;margin-bottom:5px; width: 90%;"
                                                                       placeholder="$"
                                                            ...
                                                            <input type="hidden" name="amountPaidID"
                                                                   value="{{$contract->id}}">
                                                            <input type="text" class="amountPaid" name="amountPaid[]"
                                                                   value="${{number_format($physician->remaining_amount, 2,'.','')}}"
                                                                   style="margin-top:5px;margin-bottom:5px;width: 90%;color:{{$physician->color}}; {{$physician->remaining_amount == 0 && count($physician->amountPaid) > 0 ? "display:none;":""}}"
                                                                   placeholder="$"
                                                                   title="Remaining Amount Owed" {{$disabled}}>
                                                            <input style="{{$physician->remaining_amount == 0 ? "display:none;":""}}"
                                                                   type="checkbox" name="invoice_print_checkbox_new[]"
                                                                   id="printNewAmount_{{ $physician->contract_id }}_{{ $practice->id }}"
                                                                   value="{{$physician->id}}" checked {{$disabled}}>
                                                        @endif
                                                        @if($index == 0)
                                                            <input type="hidden" class="remaining_amt_{{$contract->id}}"
                                                                   id="remaining_amt_{{$contract->id}}"
                                                                   value="${{number_format($physician->remaining_amount, 2,'.','')}}">
                                                            <input type="hidden"
                                                                   class="remaining_amt_changed_{{$contract->id}}"
                                                                   id="remaining_amt_changed_{{$contract->id}}"
                                                                   value="${{number_format($physician->remaining_amount, 2,'.','')}}">
                                                        @endif
                                                    </div>
                                                    <input type="hidden" name="prevval" value="{{$preval}}"
                                                           id="{{$contract->id}}">
                                                    <input type="hidden" class="paymentDone"
                                                           id="paymentDone{{$contract->id}}" name="paymentDone"
                                                           value="{{$paymentDone}}">
                                                    <div class="alert alert-danger"
                                                         style="display:none;padding:5px;clear:both;margin-top:10px;">
                                                    ...
                                                    <input type="hidden"
                                                           class="worked_hour_contract_id_{{$contract->id}} pull-right"
                                                           name="amountPaid[]"
                                                           value="{{$physician->worked_hours}}">
                                                    <div class="alert alert-danger"
                                                         style="display:none;padding:5px;clear:both;margin-top:10px;">
                                                    ...
                                                    <div class="alert alert-danger"
                                                         id="invoiceDismissContract_{{$physician->contract_id}}"
                                                         style="display:none;padding:5px;clear:both;margin-top:10px;">
                                                        <a class="close" id="invoiceErrorDismiss">&times;</a>
                                                        <strong>Error!</strong> There is no invoice note present for
                                                        this contract.
                                                    </div>
                                                    <div class="alert alert-danger"
                                                         id="invoiceDismissPhysician_{{$physician->id}}"
                                                         style="display:none;padding:5px;clear:both;margin-top:10px;">
                                                        <a class="close" id="invoiceErrorDismiss">&times;</a>
                                                        <strong>Error!</strong> There is no invoice note present for
                                                        this physician.
                                                    ...
                                                    @if(($contract->payment_type_id)==2)
                                                        <input type="hidden" class="pull-right"
                                                               name="maxAnnual_{{$physician->contract_id}}"
                                                               id="maxAnnual_{{$physician->contract_id}}"
                                                               value="{{$physician->annual_max_pay}}">
                                                        <div class="alert alert-danger"
                                                             style="display:none;padding:5px;clear:both;margin-top:10px;">
                                                            <a class="close" id="maxHoursDismiss">&times;</a>
                                                            This user has reached their Maximum Paid Amount for this
                                                            Year. Max
                                                            possible payment: <span
                                                                    id="physicianAnnual_{{$physician->contract_id}}">{{$physician->annual_max_pay}}</span>
                                                        </div>
                                                    @endif
                                            ...
                                            <div class="col-md-{{$final_payment_column_row}}"
                                                 style="padding-left: 0px; color: black;"><strong>Final Payment
                                                    : </strong></div>
                                            <input style="" type="checkbox" name="invoice_final_payment_checkbox_new[]"
                                                   id="finalPayment_{{ $physician->contract_id }}_{{ $practice->id }}"
                                                   value="{{$physician->id}}" {{$checked}} {{$disabled}}>
                                            <div style="clear: both;"></div>

                                            <div class="col-md-{{$expected_contract_total_col_row}}"
                                                 style="padding-left: 0px; padding-right: 0px; color:{{$physician->remaining_amount >= 0 ? "black": "red"}};">
                                                <strong>Expected Contract Total : </strong>
                                                <span class="remaining {{$contract->id}}">${{number_format($physician->remaining_amount,2,'.','')}}</span>
                                            </div>
                                            @if($contract->is_shared_contract == 0)
                                                <input type="hidden" class="expected_total_{{$contract->id}}"
                                                       value="{{number_format($physician->remaining_amount + $preval,2,'.','') }}">
                                            @endif
                                            <div style="clear: both;"><br/></div>
                                        </ul>
                                    </div>
                                    ...
                                    @if($contract->is_shared_contract == 1)
                                        <div style="width: 45%; float: left;">
                                            ...
                                            @foreach ($practice->amountPaid as $amount_paid)
                                                ...
                                                <input type="text" id="amountPaidId_{{$amount_paid->id}}"
                                                       class="prevAmount prevAmount_{{ $contract->id }}"
                                                       name="amountPaidt"
                                                       value="${{number_format($amount_paid->amountPaid, 2,'.','')}}"
                                                       style="margin-top:5px;margin-bottom:5px; width: 90%;"
                                                       placeholder="$" title="Amount Paid" {{$disabled}}>
                                                    <?php /* @if($physician->remaining_amount > 0) */ ?>
                                                <input type="checkbox" name="invoice_print_checkbox_paid[]"
                                                       id="printPaidAmount_{{$amount_paid->id}}"
                                                       value="{{$amount_paid->id}}" {{$disabled}}>
                                                    <?php /*@else
                                                    <input style="display:none;" type="checkbox" name="invoice_print_checkbox_paid[]" id="printPaidAmount_{{$amount_paid->id}}" value="{{$amount_paid->id}}" checked>
                                                ...
                                                */ ?>
                                                <input type="hidden"
                                                       class="amtPaidId_{{$contract->id}}_{{$practice->id}}"
                                                       id="amtPaidId_{{$contract->id}}_{{$practice->id}}"
                                                       value="{{$amount_paid->id}}">
                                                <input type="hidden" id="invoiceNo_{{$amount_paid->id}}"
                                                       value="{{$amount_paid->invoice_no}}">
                                                <input type="hidden" id="amountPaidIdOld_{{$amount_paid->id}}"
                                                       class="prevAmountOld prevAmountOld_{{ $contract->id }}"
                                                       name="amountPaidOld"
                                                       value="${{number_format($amount_paid->amountPaid, 2,'.','')}}"
                                                       style="margin-top:5px;margin-bottom:5px; width: 90%;"
                                                       placeholder="$" title="Amount Paid" {{$disabled}}>
                                            ...
                                            <input type="text" class="amountPaid" name="amountPaid[]"
                                                   value="${{number_format($practice->remaining_amount, 2,'.','')}}"
                                                   style="margin-top:5px;margin-bottom:5px;width: 90%;color:{{$practice->color}}; {{$practice->remaining_amount == 0 && count($practice->amountPaid) > 0 ? "display:none;":""}}"
                                                   placeholder="$" title="Remaining Amount Owed">
                                            <input style="{{$practice->remaining_amount == 0 ? "display:none;":""}}"
                                                   type="checkbox" name="invoice_print_checkbox_new[]"
                                                   id="printNewAmount_{{ $contract->id }}_{{ $practice->id }}"
                                                   value="{{$contract->id}}" checked {{$disabled}}>
                                        </div>
                                        <input type="hidden" name="prevval" value="{{$preval}}" id="{{$contract->id}}">
                                        <input type="hidden" class="paymentDone" id="paymentDone{{$contract->id}}"
                                               name="paymentDone" value="{{$paymentDone}}">
                                        <input type="hidden" class="expected_total_{{$contract->id}}"
                                               value="{{number_format($practice->remaining_amount + $preval,2,'.','') }}">
                                        <div class="alert alert-danger"
                                             style="display:none;padding:5px;clear:both;margin-top:10px;">
                                            <a class="close" id="paymentDismiss">&times;</a>
                                            <strong>Error!</strong> Please Enter Valid Payment.
    ...
    <div class="form-group">
        <div class="col-xs-5 control-label" style="padding-left: 0px;padding-right: 0px;">
        ...
        <div class="col-xs-6" style="float: right;">
            <button type="button" id="submit_pmt" class="btn btn-primary actionBtn"
                    style="float: right;margin-top:0px !important;">Submit
                Payment
            </button>
        </div>
    </div>
    ...
    <!-- Confirmation Modal -->
    <div class="modal fade" id="invoicePreview" tabindex="-1" role="dialog" aria-labelledby="invoicePreviewlLabel"
         aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background: #DCDCDC;">
                    ...
                    <span style='margin-left: 3px;'> <i class="bi bi-square-fill"
                                                        style='color: #F3DC0D; margin-right: 3px;'></i> Modified</span>
                    <span style='margin-left: 3px;'> <i class="bi bi-square-fill"
                                                        style='color: #04AA6D; margin-right: 3px;'></i>  New</span>
                </div>
                <div class="modal-body">
                    No preview is available
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal" style="background: #d43f3a">
                        Cancel
                    </button>
                    <button type="button" id="ajax_submit" class="btn btn-primary" style="background: #f68a1f">Confirm
                    </button>
                </div>
            </div>
        </div>
...
<script>
    function fetch_selected_data(id) {
        ...
        if (id == "start_date") {
            var redirectURL = "?a_id=" + agrrement_id + "&p_id=" + practice_id + "&t_id=" + type_id + "&ct_id=" + contract_type_id + "&phy_id=" + physician_id + "&m_id=" + monthNumber;
        } else if (id == "physician_id") {
            var redirectURL = "?a_id=" + agrrement_id + "&p_id=" + practice_id + "&t_id=" + type_id + "&ct_id=" + contract_type_id + "&phy_id=" + physician_id + "&m_id=" + monthNumber;
        } else if (id == "contract_type_id") {
            var redirectURL = "?a_id=" + agrrement_id + "&p_id=" + practice_id + "&t_id=" + type_id + "&ct_id=" + contract_type_id + "&m_id=" + monthNumber;
        } else if (id == "payment_type_id") {
            var redirectURL = "?a_id=" + agrrement_id + "&p_id=" + practice_id + "&t_id=" + type_id + "&m_id=" + monthNumber;
        } else if (id == "practice_id") {
            var redirectURL = "?a_id=" + agrrement_id + "&p_id=" + practice_id + "&m_id=" + monthNumber;
        } else if (id == "agreement_id") {
            var redirectURL = "?a_id=" + agrrement_id;
        }
    ...
    $("#submit_pmt").click(function (event) {
        ...
        $(".contract_id").each(function () {
            // debugger;
            var contract_id = $(this).val();
            var practice_name = $("#practice_name_" + contract_id).val();
            var practice_id = $("#practice_id_" + contract_id).val();
            var physician_name = "";

            $(".physician_id_" + contract_id).each(function (key, value) {
            ...
            // var physician_id = $(this).val();
            // var contract_id = $("#contract_id_"+physician_id).val();
            // var physician_name = $("#physician_name_"+physician_id).val();
            var final_pmt_done = $("#finalPayment_" + contract_id + "_" + practice_id).prop('disabled');

            var invoice_no = '-';
            $(".amtPaidId_" + contract_id + "_" + practice_id).each(function () {
            ...
            var owed_amount = $(".remaining_amt_" + contract_id).val();
            var remaining_amount = $(".remaining_amt_changed_" + contract_id).val();
            var print_invoice_new = $("#printNewAmount_" + contract_id + "_" + practice_id).prop('checked');
            // var is_final_pmt = $("#finalPayment_"+physician_id+"_"+contract_id+"_"+practice_id).prop('checked');
            var prevAmt = '-';
        ...
        $("#invoicePreviewlLabel").html('<h5 class="modal-title" id="invoicePreviewlLabel">Invoice Details :' + '<br/>' + agreement_name + '<br/>' + period + '<br/>' + 'Print Combined Invoice : ' + print_all_invoice + '</h5>');
    ...
    $("#ajax_submit").click(function (event) {
        ...
        $(".prevAmount").each(function () {
            ...
            prevAmountArray.push({id: paidId, amount: getAmount($(this).val()), print: print});
        ...
        $(".amountPaid").each(function () {
// debugger;
            var finalpayment = $('#finalPayment_' + tempContractIdArray[amt_count] + '_' + tempPracticeIdArray[amt_count] + ':checkbox:checked').length > 0;
            var finalpayment_disabled = $('#finalPayment_' + tempContractIdArray[amt_count] + '_' + tempPracticeIdArray[amt_count] + ':checkbox:disabled').length > 0;

            if (finalpayment_disabled == false) {
                amountPaidArray.push(getAmount($(this).val()));

                var print = $('#printNewAmount_' + tempContractIdArray[amt_count] + '_' + tempPracticeIdArray[amt_count] + ':checkbox:checked').length > 0;
        ...
        if (errorShown) {
            ...
            var practiceFlag = 0;
            debugger;
            tempContractIdArray.forEach(function (id, index) {
                var cont_worked_count = $(".worked_hour_contract_id_" + id).length;
                ...
                $(".worked_hour_contract_id_" + id).each(function (worked_index, worked_val) {
                ...
                if (!paymentErrorFlag && paymentType == 2) {
                    ...
                    if (parseFloat(paymentValue) > parseFloat($('#maxAnnual_' + id).val()) && paymentValue != 0) {
                        //var errorMessage = "This user has reached their Maximum Paid Amount for this period. Max hours paid: "+maxHoursArray[count];
                        var physician = "#physicianAnnual_" + id;
                        $(physician).html("$" + $('#maxAnnual_' + id).val().toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
                        $('#maxAnnual_' + id).next().show();
                        error++;
                        if (errorFlag) {
                            $('html,body').animate({scrollTop: $('#maxAnnual_' + id).next().offset().top - '50'}, '3000');
                            errorFlag = false;
                        }
                        errorShown = true;
                    } else if (parseFloat(paymentValue) > parseFloat(maxHoursArray[index]) && paymentValue != 0) {
                        //var errorMessage = "This user has reached their Maximum Paid Amount for this period. Max hours paid: "+maxHoursArray[count];
                        var physician = "#physician" + id;
                        ...
                        if (errorFlag) {
                            $('html,body').animate({scrollTop: $(".maxHours").next().offset().top - '50'}, '3000');
                            errorFlag = false;
                        }
                        errorShown = true;
                    }
        ...
        if (error == 0) {
            $.ajax({
                ...
                data: {
                    ids: idsArr,
                    vals: dataArr,
                    contract_ids: contractIdArray,
                    practice_ids: practiceIdArray,
                    start_date: start_date[1],
                    end_date: end_date[0],
                    selected_date: start_date_v,
                    prev_values: prevAmountArray,
                    printNew: printNewArray,
                    hospital_id: hospitalId,
                    finalPayment: finalPaymentArray,
                    timestamp: timeZone,
                    timeZone: zoneName,
                    print_all_invoice_flag: print_all_invoice_flag
                },
    ...
    $(".amountPaid , .prevAmount").blur(amountTotal = function (event) {
        ...
        var prevalIdAndClass = $(this).prev().val();//contract id
        ...
        if (/^\-?([0-9]+(\.[0-9]+)?|Infinity)$/
            ...
            if ($(this).hasClass('prevAmount')) {
                ...
                $('.prevAmount_' + $(this).prev().val()).each(function (index, el) {
                    if (getAmount($(this).val()) != "" && !isNaN(getAmount($(this).val()))) {
                        total += parseFloat(getAmount($(this).val()));
                    }
                    console.log('1==' + total);
            ...
            $(this).parent().parent().siblings('li').each(function (index, el) {
                //For calculating previous amounts
                ...
                //For calculating to be paid amounts
            ...
            if ($(".is_shared_contract_" + prevalIdAndClass).val() == '1') {
                //For shared contract
                var practice_total_remain = parseFloat(getAmount($(this).parent().parent().parent().prev().prev().children('span').html())) - parseFloat(getAmount($("." + prevalIdAndClass).html()));
            } else {
                //For non-shared contract
                var practice_total_remain = parseFloat(getAmount($(this).parent().parent().parent().parent().parent().parent().prev().prev().children('span').html())) - parseFloat(getAmount($("." + prevalIdAndClass).html()));
            }

            // console.log('substract_total== '+substract_total);
            var remain = parseFloat($(".expected_total_" + prevalIdAndClass).val()) - parseFloat(substract_total);
            practice_total_remain = practice_total_remain + remain;
            $("." + prevalIdAndClass).html('$' + remain.toFixed(2));
            ...
            if ($(this).hasClass('prevAmount')) {
                ...
                var contract_id = $(this).prev().val();//contract_id
                $("#remaining_amt_changed_" + contract_id).val($(this).val());
            }
        } else {
            $(this).parent().next().next().next().show();
            $('html,body').animate({scrollTop: $(this).parent().next().next().next().offset().top - '50'}, '3000');
    ...
    $(".amountPaid").focus(amountTotal = function (event) {
        // debugger;
        var prevval = 0;
        var prevalId = $(this).prev().val(); //contract_id
        //alert($(this).val());
        if (/^\-?([0-9]+(\.[0-9]+)?|Infinity)$/
            .test(getAmount($(this).val()))) {
