...
@section("content")
    <div class="filters clearfix">
		...
		<a class="active" href="{{ URL::current() }}">Attestations</a>
        <a class="" style="display: block;" href="{{ URL::route('hospitals.paymentSummary', $hospital->id) }}">Payment summary</a>
    </div>
...
@section('scripts')
    <script type="text/javascript">
        $(function () {
            ...
            @isset($report_id)
                Dashboard.downloadUrl("{{ route('attestation.report',[$report_id]) }}");
            @endisset
            ...
            $(document).on("click", ".agreement", function(event) { Dashboard.updateAttestationReportsFormWithData(); });
            $(document).on("change", ".select_dates", function(event) { Dashboard.updateAttestationReportsFormWithData(); });
        });
    </script>
@endsection
