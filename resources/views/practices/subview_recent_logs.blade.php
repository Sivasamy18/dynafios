@if(count($recent_logs)>0)
    ...
    <?php $header_id = 0; ?>
    @if((count($hourly_summary) > 0) && ($payment_type_id == 2))
        @foreach($hourly_summary as $hourly_summary)
            <?php $header_id++; ?>
            <div id="header_{{$header_id}}" style="display:none">
                <div class="col-xs-12"
            ...
            @foreach($recent_logs as $recent_log)
                ...
                @if( $log_date >= $hourly_summary['start_date'] && $log_date <= $hourly_summary['end_date'])
                    @if($recent_log['log_physician_id'] == $phyisican_id)
                        <script>
                            $('#header_{{$header_id}}').show();
                        </script>
                                                    ...
                                                    if (!mandate_details || override_mandate_details_flag == true) {
                                                        $.ajax({
                                                            ...
                                                            if (response.status == 1) {
                                                                ...
                                                                //$("#" + log_id).remove();
                                                                //$("#" + log_id).remove();
                                                                getRecentLogs(physicianid, $('#contract_name').val());
                                                            } else {
                                                                $('#modal-confirm-edit #enterLogMessageLog_' + log_id).show();
