...
<script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Reasons', 'Count',{type: 'string', role: 'tooltip'} @if(count($type_data) > 0) ,{type: 'number', role: 'id'} @endif],
                    @if(count($type_data) > 0)
                        @foreach ($type_data as $reason_stat)
                            ['{{$reason_stat['reason_name']}}', {{$reason_stat['rejection_count']}},'{{$reason_stat['reason_name'].' \n'.$reason_stat['rejection_count'].' '}}',{{$reason_stat['reason_id']}}],
                        @endforeach
                    @else
            ...
            var chart = new google.visualization.PieChart(document.getElementById('donutchart_reason'));
