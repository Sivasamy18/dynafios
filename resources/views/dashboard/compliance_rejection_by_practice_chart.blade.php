...
<script type="text/javascript">
	...
	@if(count($type_data) > 0)
		cols: [
			{label: 'X', type: 'string'},
			@foreach ($type_data as $practice_stat)
				{label: '{{$practice_stat['practice_name']}}'.replaceAll('&amp;', '&').replaceAll('&lt;', '<').replaceAll('&gt;', '>').replaceAll('&#039;', "'"), type: 'number'},
		...
		rows: [
			{c:[{v: ''},
				@foreach ($type_data as $practice_stat)
					{ v: {{$practice_stat['rate']}}},
					{ v: '{{$practice_stat['practice_name']}}'.replaceAll('&amp;', '&').replaceAll('&lt;', '<').replaceAll('&gt;', '>').replaceAll('&#039;', "'")},
					{ v: {{$practice_stat['practice_id']}}},
				@endforeach
			]},
		]
	@else
  ...
  var chart = new google.visualization.BarChart( document.getElementById('donutchart_practice') );
...
function selectHandler3() {
	...
	if (selectedItem) {
		...
		$('.modal-title').html('Rejection Rate By Practice '+topping);
		$('div#pop-up').show();
		...
		$('.loaderPopup').show();
		$.ajax({
			url:'/getComplianceRejectionByPractice/'+$("#hospital").val()+'/'+ typeID,
			type:'get',
