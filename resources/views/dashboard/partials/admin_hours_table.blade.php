    ...
    <table class="table table-striped table-hover hospitals-table">
        ...
        @foreach ($items as $contract)
            <tr>
                ...
                <td class="text-center rowlink-skip" onClick="editAdminHours('{{$contract->contract_id}}','{{$contract->physician_name}}', '{{$contract->agreement_name}}',
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Add/Update Additional Admin Hours</h4>
            </div>
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        @csrf
                            ...
                            <div class="form-group row">
                                <label for="weekly-max-hours" class="col-sm-4 col-form-label">Additional Admin Hours</label>
                                <div class="col-sm-4">
                ...
                <p><strong style="color: red">Warning!</strong><br>
                    This action will add/update the additional admin hours for the current month.
                </p>

            </div>
            <div class="modal-footer">
                <button type="button" data-dismiss="modal">Cancel</button>
                <button type="button" id="save-max-hour" onclick="saveAdminHours()">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function saveAdminHours(){
        ...
        $.ajax({
            type:"post",
            dataType: "json",
            data: {contract_id: contract_id, selected_date:selected_date, admin_hours:max_hours},
            url:'/postAdminHours',
            success:function(response){
                if(response && Object.keys(response).length != 0){
                    $(".alert-success").append(response.message);
                    $(".alert-success").show();
                    $('#weekly-max-hours').val(parseFloat(response.admin_hours).toFixed(2));
                    setTimeout(function() {
    ...
    $('#months').change(function(){
        ...
        $.ajax({
            type:"get",
            url:'/getAdminHours/' + contract_id + '/' +selected_date,
            success:function(response){
                if(response && Object.keys(response).length != 0){
                    $('#weekly-max-hours').val(parseFloat(response.admin_hours).toFixed(2));
                } else {
    ...
    function editAdminHours(contract_id, physician_name, agreement_name, contract_type_name, contract_name, hospital_name){
        ...
        var today = new Date();
        // var current_month_start_date = new Date(today.getFullYear(), today.getMonth(), 1); // Should be used in production.
        var current_month_start_date = new Date(today.getFullYear(), today.getMonth() - 3, 1); // Should be used for testing.

        for(let i=1; i<=3; i++){ // Should be commented for production only uncomment while testing.
            // current_month_start_date.setMonth(current_month_start_date.getMonth()); // Should be commented for production only uncomment while testing.
            current_month_start_date.setMonth(current_month_start_date.getMonth() + 1); // This is for testing only otherwise comment this line
