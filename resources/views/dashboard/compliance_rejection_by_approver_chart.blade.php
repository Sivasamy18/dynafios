<script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Approvers', 'Count',{type: 'string', role: 'tooltip'} @if(count($type_data) > 0) ,{type: 'number', role: 'id'} @endif],
                    @if(count($type_data) > 0)
                        @foreach ($type_data as $approver_stat)
                            ['{{$approver_stat['approver_name']}}', {{$approver_stat['rejection_count']}},'{{$approver_stat['approver_name'].' \n'.$approver_stat['rejection_count'].' '}}',{{$approver_stat['approver_id']}}],
                        @endforeach
                    @else
            ...
            var chart = new google.visualization.PieChart(document.getElementById('donutchart_approver'));
