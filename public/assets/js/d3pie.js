...
var defaultSettings = {
	...
	misc: {
		colors: {
			background: null,
			segments: [
				"#f2f2f2", "#5c5247", "#d2c2b3", "#d8d23a", "#e98125", "#d0743c", "#515151", "#947E6E", "#B5A790", "#6A5A4E", "#564940",
...
var labels = {
	...
	add: function(pie, section, sectionDisplayType) {
		...
		if (include.amount) {
			labelGroup.append("text")
				.attr("id", function(d, i) { return pie.cssPrefix + "segmentAmount" + i + "-" + section; })
				.attr("class", pie.cssPrefix + "segmentAmount-" + section)
				.text(function(d, i) {
					var amount = numeral(d.value).format('$ 0,0[.]00');
					if (settings.formatter) {
						formatterContext.index = i;
						formatterContext.part = "amount";
						formatterContext.value = d.value;
						formatterContext.label = d.value;
						amount = settings.formatter(formatterContext);
					} else {
						amount = amount;
					}
					return amount;
				})
				.style("font-size", settings.amount.fontSize + "px")
				.style("font-family", settings.amount.font)
				.style("fill", settings.amount.color);
		}
        //Chaitraly::Added for hours performance dashboard
		if (include.hour) {
			labelGroup.append("text")
				.attr("id", function(d, i) { return pie.cssPrefix + "segmentHour" + i + "-" + section; })
				.attr("class", pie.cssPrefix + "segmentHour-" + section)
				.text(function(d, i) {
					var hrs = " hours";
					var hour = numeral(d.value).format("0,0[.]00")+hrs;
					if (settings.formatter) {
						formatterContext.index = i;
						formatterContext.part = "hour";
						formatterContext.value = d.value;
						formatterContext.label = d.value;
						hour = settings.formatter(formatterContext);
					} else {
						hour = hour;
					}
					return hour;
				})
				.style("font-size", settings.hour.fontSize + "px")
				.style("font-family", settings.hour.font)
				.style("fill", settings.hour.color);
		}
	},
	...
	positionLabelElements: function(pie, section, sectionDisplayType) {
		...
		switch (sectionDisplayType) {
			...
			case "label-percentage2":
				...
				break;
			case "label-amount1":
				d3.selectAll("." + pie.cssPrefix + "segmentAmount-" + section)
					.attr("dy", function(d, i) { return dims[i].mainLabel.height; });
				break;
			case "label-hour1":
				d3.selectAll("." + pie.cssPrefix + "segmentHour-" + section)
					.attr("dy", function(d, i) { return dims[i].mainLabel.height; });
				break;
	 	}
	},
