...
use App\AccessToken;
use App\ApiGraph;
use App\customeClasses\ProductivMD;
use App\Physician;

class ApiGraphController extends BaseController
{
    const STATUS_FAILURE = 0;
    const STATUS_SUCCESS = 1;

    public function getProviderByNPI()
    {
        ...
        $productivmd = new ProductivMD();
        $results = $productivmd->getProviderByNPI($npi);
    ...
    public function getCompensationSummary()
    {
        ...
        $accrualInterval = Request::input("accrualInterval");     // annual / monthly / semi-annual / quarterly
        ...
        $productivmd = new ProductivMD();
        $results = $productivmd->getCompensationSummary($providerId, $startDate, $endDate, $accrualInterval, $physician);
    ...
    public function getProductivityCompensation()
    {
        $token = Request::input("token");
        $token = json_decode($token);
        $access_token = AccessToken::where("key", "=", $token)->first();
        ...
        $productivmd = new ProductivMD();
        $results = $productivmd->getProductivityCompensation($providerId, $year, $month, $physician);
    ...
    public function getTtmProductivity()
    {
        $token = Request::input("token");
        $token = json_decode($token);
        $access_token = AccessToken::where("key", "=", $token)->first();
        ...
        $productivmd = new ProductivMD();
        $results = $productivmd->getTtmProductivity($providerId, $year, $month, $physician);
