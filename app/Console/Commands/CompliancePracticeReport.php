...
class CompliancePracticeReport extends ReportingCommand
{
	protected $name = "reports:compliancePracticeReport";
	protected $description = "Generates a DYNAFIOS practice complience report.";
	...
	public function __invoke()
	{
		...
		$workbook = $reader->load(storage_path()."/reports/templates/practice_compliance_report.xlsx");
		...
		foreach($arguments->report_data as $hospitalData){
			...
			$header .= "Practice Compliance Report\n";
...
//			$current_row++;
			foreach($hospitalData["agreementInfo"] as $data) {
				if(count($data['practice_info']) > 0) {
					...
					foreach ($data['practice_info'] as $practiceInfo) {
						$workbook->setActiveSheetIndex($sheetIndex)
							->setCellValue("B{$current_row}", $practiceInfo["practice"])
							->setCellValue("C{$current_row}", $practiceInfo["period"])
							->setCellValue("D{$current_row}", $practiceInfo["submit_count"])
							->setCellValue("E{$current_row}", $practiceInfo["reject_count"])
							->setCellValue("F{$current_row}", formatNumber($practiceInfo["rejection_percent"]).'%')
							->setCellValue("G{$current_row}", $practiceInfo["submit_cytd"])
							->setCellValue("H{$current_row}", $practiceInfo["reject_cytd"])
							->setCellValue("I{$current_row}", formatNumber($practiceInfo["rejection_percent_cytd"]).'%')
					...
					$workbook->setActiveSheetIndex($sheetIndex)
						->setCellValue("D{$current_row}", $data["total_logs"])
						->setCellValue("E{$current_row}", $data["total_rejection_log"])
						->setCellValue("F{$current_row}", formatNumber($data["total_rejection_percent"]).'%')
						->setCellValue("G{$current_row}", $data["total_log_cytd"])
						->setCellValue("H{$current_row}", $data["total_rejection_log_cytd"])
						->setCellValue("I{$current_row}", formatNumber($data["total_rejection_percent_cytd"]).'%')
						->setCellValue("J{$current_row}", $data["benchmark"].'%');
					$workbook->setActiveSheetIndex($sheetIndex)->getStyle("B{$current_row}:J{$current_row}")->applyFromArray($this->cell_style);
					$workbook->setActiveSheetIndex($sheetIndex)->getStyle("C{$current_row}:J{$current_row}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
					$current_row++;
				}
		...
		//$report_filename = "compliance_practice_" . date('mdYhis') . ".xlsx";
		$timeZone = str_replace(' ','_', $hospitalData['localtimeZone']);
        $timeZone = str_replace('/','', $timeZone);
        $timeZone = str_replace(':','', $timeZone);
		$report_filename = "compliance_practice_" . $timeZone . ".xlsx";
		...
		$compliance_report->report_type = 2;
